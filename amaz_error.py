# -*- coding: utf-8 -*-

import urllib
import urllib2
import zlib
import re, datetime
from bs4 import BeautifulSoup
from model.amazon_errcode import Amazon_errcode
from model.session import sessionCM
import sys
reload(sys)
sys.setdefaultencoding("utf-8")

class Amazon_errmsg(object):

    def __init__(self):
        self.postdata = urllib.urlencode({
            "widgetToken": ":cGxwbE14MEVhUnlDVENweTNueEpPZXhkbUtmMEwwT2tKRnRjcUlWK3kyWT06MQ==",
            "rememberMe": "false",
            "username": "yangguoli@actneed.com",
            "password": "yaya2015",
            "sign-in-button": "",
            "metadata1": "",
        })
        self.headers = {
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8",
            "Accept-Encoding": "gzip, deflate, sdch",
            "Accept-Language": "zh-CN,zh;q=0.8,en;q=0.6",
            "Connection": "keep-alive",
            "Cookie": 'aws-target-static-id=1460341173658-788740; aws-target-visitor-id=1460341173661-21096; aws-target-data=%7B%22support%22%3A%221%22%7D; s_fid=134492BDA8807349-20AB2D7C815E1F64; s_dslv=1460341175322; s_vn=1491877175325%26vn%3D1; s_nr=1460341175326-New; regStatus=pre-register; csm-hit=696.68|1463450295274; s_pers=%20s_ev15%3D%255B%255B%2527Typed/Bookmarked%2527%252C%25271463356822486%2527%255D%252C%255B%2527Typed/Bookmarked%2527%252C%25271463356824667%2527%255D%252C%255B%2527Typed/Bookmarked%2527%252C%25271463370142563%2527%255D%252C%255B%2527Typed/Bookmarked%2527%252C%25271463379458205%2527%255D%255D%7C1621145858205%3B%20s_dl%3D1%7C1463477459715%3B%20gpv_page%3DUS%253ASC%253A%2520SellerCentralLogin%7C1463477459718%3B; s_sess=%20c_m%3DundefinedTyped/BookmarkedTyped/Bookmarked%3B%20ev1%3Dn/a%3B%20s_cc%3Dtrue%3B%20s_sq%3Damznsrvsprod%252Camznsrvsmainprod%253D%252526pid%25253DUS%2525253ASC%2525253A%25252520SellerCentralLogin%252526pidt%25253D1%252526oid%25253D%252525A0%252525A0%252525u767B%252525u5F55%252525A0%252525A0%2525250A%252526oidt%25253D3%252526ot%25253DSUBMIT%3B; session-id=180-9662177-9190219; session-id-time=2094195590l; ubid-main=187-8952156-1332319; session-token="QMOIJ6X3w0ktoIc8iqflXNpEn94a7kJ73/CFBDgELsU/+LL6mkEpvZpEA3sOK7x1D7INAOdPDTA8LbP9RrCLEeoXIKibkWq3G+ooY7EeEGfbs/T85C0+Ll2j/njs6B9N2f0X0LOjPOnXlgf5qz7FRHzJqraC0Et6wZIsojD4YqGmXHJ6nFbOFZ+k6yOMB20Q/2EoJImrXn0aom3Enj/eLfcx08FrCo2YELx6VleasNI="; x-main="6oJRhZ@DdQkUcqw1E0CPEu6zU9a7uVP?fJUpwB0wrh9ifo3G2aOT0fZXqRZmIMip"; at-main=Atza|IQEBLzAtAhRAlVXoekO3dZE_kQEfSGhSQKSw-gIVAIaFMLeOI2XZdEMKWDqKk6Y1d2cYt2Uq7tqyUNJ0Wn62yV9FaWq4xu72JteoYyJ1EVNdj_y2L23u5gBf5ytuTcSgKuvMiBECiTiNL6lyp_PThBfMbUlnHhIEryRGRv9aVNxjmU8iF70wF67Gz7b3a23yjcQGeayPA64KpaN2GOb2AlwBJpX6bFMy0F40xVnHx7__jwvWoOkluii5t_u7SgOIeHmoHe65l0M3EBrZtA_Nm25XqlY4v1g9njU0FE07b4nUYEm6QD90gc9pVdUJIpVJPetKPFhJBrCAWioCd6lqxqrgH09ugweUs5PyQZ1VqLkhNA39; sess-at-main="98yEhxaYabcHf5z9cLtASFgLp3IEeiHk0O5ajxiBMOs="',
            "Host": "sellercentral.amazon.com",
            "Referer": "https://sellercentral.amazon.com/gp/homepage.html?ie=UTF8&*Version*=1&*entries*=0&",
            "Upgrade-Insecure-Requests": "1",
            "User-Agent": "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.94 Safari/537.36"
        }

    def get_list_html(self, list_link):
        print len(list_link), "----type--list_link--", type(list_link)
        html_list = []
        for i in range(len(list_link)):
            req = urllib2.Request(
                url=list_link[i],
                data=self.postdata,
                headers=self.headers
            )
            result = urllib2.urlopen(req)
            gzipped = result.headers.get('Content-Encoding')
            if gzipped:
                htmls = zlib.decompress(result.read(), 16+zlib.MAX_WBITS)
                # print "get_list_html-----", htmls
                html_list .append(htmls)
        return html_list

    def get_list(self, html):
        list_link = []
        for i in range(len(html)):
            soup = BeautifulSoup(html[i])
            ul = soup.select("#subtopics a")
            if ul:
                for u in range(len(ul)):
                    link_a = ul[u]
                    # print type(link_a)
                    # print link_a.get("href")
                    # print link_a.string
                    list_link.append(link_a.get("href"))
                # print list_link
                html_list = self.get_list_html(list_link)
                self.get_list(html_list)
        print "list_link------------", list_link
        return list_link

    def get_info(self, html):
        err_solution = ""
        err_desc = ""
        err_code = 0
        p_error = ""
        pattern = re.compile(r'([0-9]+)')
        for i in range(len(html)):
            soup = BeautifulSoup(html[i])
            try:
                err_code = pattern.findall(soup.h2.string)
                print "page===", i, "------", err_code[0]
                err_desc = soup.find("p", {"class": "code"}).get_text()
                # print "err_desc=======", err_desc
                p_error = soup.find("div", {"id": "_modules"})
            except AttributeError or TypeError as e:
                print "h2, class=code, p_error is not found."
            if p_error and err_code and err_desc:
                for name in p_error:
                    try:
                       err_solution = name.get_text().replace(err_desc, "")
                    except AttributeError, e:
                        print "type---", type(name), name
                        # print "err_solution=======", err_solution.replace(" ", "")
                with sessionCM() as session:
                    try:
                        Amazon_errcode.create(session, err_code, err_desc, err_solution)
                    except AttributeError as e:
                        print "insert filed."
            else:
                print "It isn't final page."

if __name__ == "__main__":
    first = Amazon_errmsg()
    # url=['https://sellercentral.amazon.com/gp/help/help.html/ref=ag_200712450_cont_17781?ie=UTF8&itemID=200712450&language=zh_US']
    url20000 = ['https://sellercentral.amazon.com/gp/help/help.html/ref=ag_200712530_cont_17781?ie=UTF8&itemID=200712530&language=zh_US']
    url8000 = ['https://sellercentral.amazon.com/gp/help/help.html/ref=ag_201440690_cont_17781?ie=UTF8&itemID=201440690&language=zh_US']
    url90000 = ['https://sellercentral.amazon.com/gp/help/help.html/ref=sc_hp_rel_200712050?ie=UTF8&itemID=200712050&language=zh_US']
    url13000 = ['https://sellercentral.amazon.com/gp/help/help.html/ref=sc_hp_rel_200712510?ie=UTF8&itemID=200712510&language=zh_US']
    url1003 = ['https://sellercentral.amazon.com/gp/help/help.html/ref=sc_hp_rel_24761?ie=UTF8&itemID=24761&language=zh_US']
    url18000 = ['https://sellercentral.amazon.com/gp/help/help.html/ref=sc_hp_rel_200712520?ie=UTF8&itemID=200712520&language=zh_US']
    start_time = datetime.datetime.now()
    print "start--", start_time
    html = first.get_list_html(url1003)
    print "html--", len(html)
    # aim_list = first.get_list(html)
    # print "aim list len----", len(aim_list)
    html_time = datetime.datetime.now()
    print html_time, "***", html_time - start_time
    # aim_list_html = first.get_list_html(aim_list)
    print "html time---", datetime.datetime.now() - html_time

    first.get_info(html)
    end_time = datetime.datetime.now()
    print "start--", end_time
    print "time==", end_time - start_time
